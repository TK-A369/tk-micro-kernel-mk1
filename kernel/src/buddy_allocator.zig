const BuddyAllocator = struct {
    ...
    pub fn clear(self: *BuddyAllocator) void {
        for (0.., self.bitmaps) |i, bitmap| {
            if (i == self.bitmaps.len - 1) {
                ...
                var last_bitgroup_used_bits = self.blocks_count >> self.bitmaps.size % 32;
                if (last_bitgroup_used_bits == 0) {
                    last_bitgroup_used_bits = 32;
                }
                const last_bitgroup_padding_bits = 32 - last_bitgroup_used_bits;
    ...
    pub fn alloc(self: *BuddyAllocator, size: u64) error{OutOfMemory}![*]u8 {
        ...
        while (curr_bitmap_num > bitmap_num) : (curr_bitmap_num -= 1) {
            // Mark the larger block as used (zero)
            self.bitmaps[curr_bitmap_num][bit_num / 32] &= ~(1 << (bit_num % 32));
            bit_num *= 2;
            // Mark two child blocks as free (ones)
            self.bitmaps[curr_bitmap_num - 1][bit_num / 32] &= ~(1 << (bit_num % 32));
            self.bitmaps[curr_bitmap_num - 1][(bit_num + 1) / 32] &= ~(1 << ((bit_num + 1) % 32));
        }
